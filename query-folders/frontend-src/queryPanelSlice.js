...
import { dataqueryService, workflowExecutionsService, queryFolderService } from '@/_services';
...
export const createQueryPanelSlice = (set, get) => ({
  queryPanel: {
    ...
    fetchQueryFolders: (appVersionId) => {
      queryFolderService.getAll(appVersionId).then((data) => {
        set(
          (state) => {
            state.queryPanel.queryFolders = data.query_folders || [];
          },
          false,
          'fetchQueryFolders'
        );
      }).catch((error) => {
        console.error('Failed to fetch query folders:', error);
      });
    },

    createQueryFolder: (name) => {
      const appVersionId = get().currentVersionId;
      queryFolderService.create(name, appVersionId).then((folder) => {
        set(
          (state) => {
            state.queryPanel.queryFolders = [...state.queryPanel.queryFolders, folder];
            state.queryPanel.creatingFolder = false;
            state.queryPanel.expandedFolderIds = [...state.queryPanel.expandedFolderIds, folder.id];
          },
          false,
          'createQueryFolder'
        );
      }).catch((error) => {
        toast.error(`Failed to create folder: ${error.message ?? error.error}`);
        set((state) => { state.queryPanel.creatingFolder = false; });
      });
    },

    renameQueryFolder: (id, name) => {
      set(
        (state) => {
          state.queryPanel.queryFolders = state.queryPanel.queryFolders.map((f) =>
            f.id === id ? { ...f, name } : f
          );
          state.queryPanel.renamingFolderId = null;
        },
        false,
        'renameQueryFolder'
      );
      queryFolderService.update(id, { name }).catch((error) => {
        toast.error(`Failed to rename folder: ${error.message ?? error.error}`);
      });
    },

    deleteQueryFolder: (id) => {
      const folders = get().queryPanel.queryFolders;
      set(
        (state) => {
          state.queryPanel.queryFolders = state.queryPanel.queryFolders.filter((f) => f.id !== id);
        },
        false,
        'deleteQueryFolder'
      );
      queryFolderService.del(id).then(() => {
        // Queries that were in this folder now have null folder_id - update local state
        set((state) => {
          const moduleId = 'canvas';
          state.dataQuery.queries.modules[moduleId] = state.dataQuery.queries.modules[moduleId].map((q) =>
            q.query_folder_id === id ? { ...q, query_folder_id: null, folder_position: 0 } : q
          );
        });
      }).catch((error) => {
        toast.error(`Failed to delete folder: ${error.message ?? error.error}`);
        // Restore folders on error
        set((state) => { state.queryPanel.queryFolders = folders; });
      });
    },
    ...
    toggleFolderExpanded: (id) => {
      set(
        (state) => {
          const idx = state.queryPanel.expandedFolderIds.indexOf(id);
          if (idx >= 0) {
            state.queryPanel.expandedFolderIds = state.queryPanel.expandedFolderIds.filter((fId) => fId !== id);
          } else {
            state.queryPanel.expandedFolderIds = [...state.queryPanel.expandedFolderIds, id];
          }
        },
        false,
    ...
    setRenamingFolder: (id) => {
      set(
        (state) => {
          state.queryPanel.renamingFolderId = id;
        },
        false,
        'setRenamingFolder'
      );
    ...
    setCreatingFolder: (value) => {
      set(
        (state) => {
          state.queryPanel.creatingFolder = value;
        },
        false,
        'setCreatingFolder'
      );
    ...
    moveQueryToFolder: (queryId, folderId, position = 0) => {
      // Optimistic update
      set(
        (state) => {
          const moduleId = 'canvas';
          state.dataQuery.queries.modules[moduleId] = state.dataQuery.queries.modules[moduleId].map((q) =>
            q.id === queryId ? { ...q, query_folder_id: folderId || null, folder_position: position } : q
          );
        },
        false,
        'moveQueryToFolder'
      );
      queryFolderService.moveQuery(folderId || 'root', queryId, position, folderId).catch((error) => {
        toast.error(`Failed to move query: ${error.message ?? error.error}`);
      });
    },
