...
const QueryCardMenu = ({ darkMode }) => {
  ...
  const [showFolderSubmenu, setShowFolderSubmenu] = React.useState(false);
  ...
  const QUERY_MENU_OPTIONS = [
    ...
    },
    {
      ...
      value: 'moveToFolder',
      icon: <SolidIcon name="folder" width={16} />,
      showTooltip: false,
    },
    {
      label: 'Delete',
  ...
  const handleQueryMenuActions = (value) => {
    ...
    if (value === 'moveToFolder') {
      setShowFolderSubmenu((prev) => !prev);
      ...
      {(props) => (
        <Popover {...props} id="query-list-menu" className={darkMode && 'dark-theme'}>
          <Popover.Body bsPrefix="list-item-popover-body">
            ...
            {showFolderSubmenu && (
              <div className="query-folder-submenu-inline">
                <div
                  className="list-item-popover-option"
                  onClick={(e) => {
                    e.stopPropagation();
                    moveQueryToFolder(selectedQuery?.id, null);
                    setShowFolderSubmenu(false);
                    toggleQueryHandlerMenu(false);
                  }}
                >
                  <div className="list-item-option-menu-label">No folder</div>
                </div>
                {queryFolders.map((folder) => (
                  <div
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      moveQueryToFolder(selectedQuery?.id, folder.id);
                      setShowFolderSubmenu(false);
                      toggleQueryHandlerMenu(false);
                    }}
                  >
                    ...
                    <div className="list-item-option-menu-label">{folder.name}</div>
                  </div>
                ))}
              </div>
            )}
