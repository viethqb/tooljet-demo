...
export const QueryDataPane = ({ darkMode }) => {
  ...
  const creatingFolder = useStore((state) => state.queryPanel.creatingFolder);
  const setCreatingFolder = useStore((state) => state.queryPanel.setCreatingFolder);
  const createQueryFolder = useStore((state) => state.queryPanel.createQueryFolder);
  const moveQueryToFolder = useStore((state) => state.queryPanel.moveQueryToFolder);
  const fetchQueryFolders = useStore((state) => state.queryPanel.fetchQueryFolders);
  ...
  useEffect(() => {
    if (currentVersionId) {
      fetchQueryFolders(currentVersionId);
    }
  }, [currentVersionId]);

  // Group filtered queries by folder
  const { folderedQueries, unfiledQueries } = useMemo(() => {
    const foldered = {};
    const unfiled = [];
    filteredQueries.forEach((query) => {
      if (query.query_folder_id) {
        if (!foldered[query.query_folder_id]) foldered[query.query_folder_id] = [];
        foldered[query.query_folder_id].push(query);
      } else {
        unfiled.push(query);
      }
    });
    return { folderedQueries: foldered, unfiledQueries: unfiled };
  }, [filteredQueries]);

  // Root drop target - drop here to remove from folder
  const [{ isOverRoot }, rootDrop] = useDrop({
    accept: 'QUERY_CARD',
    drop: (item) => {
      if (item.queryFolderId) {
        moveQueryToFolder(item.queryId, null);
      }
    },
    collect: (monitor) => ({
    ...
    <div className="data-pane">
      <div className={`queries-container ${darkMode && 'theme-dark'} d-flex flex-column h-100`}>
        <div className="queries-header row d-flex align-items-center justify-content-between">
          ...
          <div className="col-auto d-flex" style={{ gap: '2px' }}>
            {!isFreezed && (
              ...
              >
                <SolidIcon name="folder" width="14" fill="var(--icons-default)" />
              </button>
            )}
        ...
        {loadingDataQueries ? (
          ...
          <div
            ref={rootDrop}
            className={cx('query-list tj-scrollbar overflow-auto', {
              'flex-grow-1 align-items-center justify-content-center': filteredQueries.length === 0,
              'root-drop-active': isOverRoot,
            })}
          >
            <div>
              {/* New folder input */}
              {creatingFolder && (
                <div className="query-folder-row creating">
                  <div className="folder-icon">
                    <SolidIcon name="folder" width="16" fill="var(--icons-default)" />
                  </div>
                  <div className="folder-name" style={{ flex: 1 }}>
                    <input
                      autoFocus
                      ...
                      onKeyDown={(e) => {
                        if (e.key === 'Enter' && newFolderName.trim()) {
                          createQueryFolder(newFolderName.trim());
                          setNewFolderName('');
                        }
                        if (e.key === 'Escape') {
              ...
              {/* Render folders sorted by position */}
              {[...queryFolders]
                ...
                .map((folder) => (
                  <QueryFolder
                    key={folder.id}
                    folder={folder}
                    queries={folderedQueries[folder.id] || []}
                    darkMode={darkMode}
                  />
                ))}

              {/* Render unfiled queries */}
              {unfiledQueries.map((query) => (
