...
export const QueryCard = ({ dataQuery, darkMode = false, localDs, inFolder = false }) => {
  ...
  const [{ isDragging }, drag] = useDrag({
    type: 'QUERY_CARD',
    item: { queryId: dataQuery.id, queryFolderId: dataQuery.query_folder_id },
    collect: (monitor) => ({
  ...
  return (
    <>
      <div
        ref={!shouldFreeze ? drag : undefined}
        className={`row query-row pe-2 ${darkMode && 'dark-theme'}` + (isQuerySelected ? ' query-row-selected' : '') + (inFolder ? ' query-row-in-folder' : '') + (isDragging ? ' query-row-dragging' : '')}
        key={dataQuery.id}
        onClick={(e) => {
